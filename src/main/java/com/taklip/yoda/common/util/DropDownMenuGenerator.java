...
import com.taklip.yoda.vo.DropDownMenuContainer;
...
public class DropDownMenuGenerator {
    ...
    /*
     * static public String generate1(HttpServletRequest request, String menu,
     * int selection, String selectionName) { DropDownMenu container =
     * (DropDownMenu) request.getAttribute(menu); return generate1(container,
     * selection, selectionName, null, null); }
     */
    static public String generate1(
            DropDownMenu ddmList[], String selections[], int selection,
            String selectionName, String anchorPrefix, String paramName) {
        String code = "";

        for (int i = 0; i < ddmList.length; i++) {
            code += generate1(
                ddmList[i], selections, String.valueOf(i), selection,
                selectionName, anchorPrefix, paramName);
        }
    ...
    static public String generate1(
            DropDownMenu ddm, String selections[], String id, int selection,
            String selectionName, String anchorPrefix, String paramName) {
        String formType = "";
            ...
            if (selection != SELECTION_NONE) {
                ...
                if (selections != null) {
                    for (int j = 0; j < selections.length; j++) {
                        if (String.valueOf(menuItem.getMenuKey()).equals(selections[j])) {
                ...
                code += "<input type=\"" + formType + "\" name=\""
                    + selectionName + "\" value=\"" + menuItem.getMenuKey()
                        + "\" " + check + ">";
            }
...
//							+ menuList[i].getMenuKey();
                    href += "/" + menuList[i].getMenuKey();
                }

                code += "<a href='"  + href + "'>" + menuList[i].getMenuName()
                        + "<div style='display:none'>"
                        + menuList[i].getMenuKey() + "</div></a>\n";
            }
            ...
            if (menuList[i].getMenuItems().length > 0) {
                code += generate1(
                    menuList[i], selections, id, selection,
                    selectionName, anchorPrefix, paramName);
            }
    ...
    static public String generateMenuItem1(DropDownMenu menuItem, String id,
            int selection, String selectionName, String anchorPrefix,
            String paramName) {
        String code = "";
        ...
        if (menuList.length > 0) {
            ...
            for (int i = 0; i < menuList.length; i++) {
                ...
                if (menuList[i].getMenuItems().length > 0) {
                    code += generateMenuItem1(
                        menuList[i], id, selection, selectionName,
                        anchorPrefix, paramName);
                }
    ...
    static public String generate(
            HttpServletRequest request, String menu, int selection,
            String selectionName) {
        DropDownMenuContainer container = 
            (DropDownMenuContainer) request.getAttribute(menu);
        return generate(container, selection, selectionName, null, null);
    }

    static public String generate(
            DropDownMenuContainer containerList[], int selection,
            String selectionName, String anchorPrefix, String paramName) {
        String code = "";
        ...
        for (int i = 0; i < containerList.length; i++) {
            ...
            if (containerList[i].getMenuSetName() != null) {
                ...
                code += "Set - "
                        + containerList[i].getMenuSetName();
            ...
            code += generate(
                containerList[i], selection, selectionName, anchorPrefix,
                paramName);
        }
    ...
    static public String generate(
            DropDownMenuContainer container, int selection,
            String selectionName, String anchorPrefix, String paramName) {
        String code = "";
        ...
        for (int i = 0; i < menuList.length; i++) {
            ...
            id += "1"
                    + "_"
                    + String.valueOf(i);
            code += generateMenuItem(
                menuList[i], id, 1, selection, selectionName, anchorPrefix,
                paramName);
        }
    ...
    static public String generateMenuItem(
            DropDownMenu menuItem, String id, int level, int selection,
            String selectionName, String anchorPrefix, String paramName) {
        String code = "";
        DropDownMenu menuList[] = menuItem.getMenuItems();
        ...
        if (menuList.length > 0) {
            code += indent
                    ...
                    + "\" class=\"sm_DDM_hide\">\n";
            code += indent
                    + "  <ul class=\"sm_DDM_container\">\n";

            for (int i = 0; i < menuList.length; i++) {
                id += "-" + String.valueOf(i);
                code += generateMenuItem(
                    menuList[i], id, level + 1, selection, selectionName,
                    anchorPrefix, paramName);
            }
