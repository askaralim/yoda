...
public class ItemServiceImpl implements ItemService {
	...
	private Item getItemFromCached(int itemId) {
		...
		try {
			...
			if (null != map && map.size() >0) {
				...
				item.setId(StringUtils.isNotEmpty(id) && !"nil".equalsIgnoreCase(id) ? Integer.valueOf(id) : null);
				item.setBuyLinks(StringUtils.isNotEmpty(buyLinks) && !"nil".equalsIgnoreCase(buyLinks) ? buyLinks : null);
				item.setCategoryId(StringUtils.isNotEmpty(categoryId) && !"nil".equalsIgnoreCase(categoryId) ? Integer.valueOf(categoryId) : null);
				item.setContentId(StringUtils.isNotEmpty(contentId) && !"nil".equalsIgnoreCase(contentId) ? Long.valueOf(contentId) : null);
				item.setDescription(StringUtils.isNotEmpty(description) && !"nil".equalsIgnoreCase(description) ? description : null);
				item.setExtraFields(StringUtils.isNotEmpty(extraFields) && !"nil".equalsIgnoreCase(extraFields) ? extraFields : null);
				item.setImagePath(StringUtils.isNotEmpty(imagePath) && !"nil".equalsIgnoreCase(imagePath) ? imagePath : null);
				item.setLevel(StringUtils.isNotEmpty(level) && !"nil".equalsIgnoreCase(level) ? level : null);
				item.setName(StringUtils.isNotEmpty(name) && !"nil".equalsIgnoreCase(name) ? name : null);
				item.setPrice(StringUtils.isNotEmpty(price) && !"nil".equalsIgnoreCase(price) ? Integer.valueOf(price) : null);
				item.setSiteId(StringUtils.isNotEmpty(siteId) && !"nil".equalsIgnoreCase(siteId) ? Integer.valueOf(siteId) : null);
				...
				item.setCreateDate(StringUtils.isNotEmpty(createDate) && !"nil".equalsIgnoreCase(createDate) ? Format.getDate(createDate) : null);
				item.setUpdateDate(StringUtils.isNotEmpty(updateDate) && !"nil".equalsIgnoreCase(updateDate) ? Format.getDate(updateDate) : null);
	...
	private void setItemIntoCached(Item item) {
		...
		value.put("buyLinks", GetterUtil.getString(item.getBuyLinks(), StringPool.BLANK));
		...
		value.put("extraFields", GetterUtil.getString(item.getExtraFields(), StringPool.BLANK));
		...
		value.put("updateDate", Format.getDate(item.getUpdateDate()));
		value.put("createBy", String.valueOf(item.getCreateBy().getUserId()));
		value.put("createDate", Format.getDate(item.getCreateDate()));
	...
	private int getItemRateFromCached(int itemId) {
		...
		if (StringUtils.isNotEmpty(rating) && !"nil".equalsIgnoreCase(rating)) {
